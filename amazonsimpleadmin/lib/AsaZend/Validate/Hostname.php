                ...
                if (preg_match('/([^.]{2,10})$/i', end($domainParts), $matches) ||
                    ...
                    if ($this->_options['idn'] &&  isset($this->_validIdns[strtoupper($this->_tld)])) {
                        if (is_string($this->_validIdns[strtoupper($this->_tld)])) {
                            if (file_exists(ASA_LIB_DIR . 'AsaZend/Validate/' . $this->_validIdns[strtoupper($this->_tld)])) {
                                $regexChars += include(ASA_LIB_DIR . 'AsaZend/Validate/' . $this->_validIdns[strtoupper($this->_tld)]);
                            } else {
